...
def mk_df(date_info, aud_usd_info, eur_aud_info):
    ...
    date_series = pd.Series([x[1] for x in date_info], [x[0] for x in date_info])
    aud_usd_series = pd.Series([x[1] for x in aud_usd_info], [x[0] for x in aud_usd_info])
    eur_aud_series = pd.Series([x[1] for x in eur_aud_info], [x[0] for x in eur_aud_info])
    ...
    df = pd.DataFrame(data = {'': date_series, 'AUD/USD': aud_usd_series, 'EUR/AUD': eur_aud_series})
    return df
